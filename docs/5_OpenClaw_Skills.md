...
![OpenClaw 实际对话截图 - system_info Skill 效果](../images/openclaw_telegram_sysinfo_demo.png)
...
```mermaid
flowchart LR
    A["🧑 用户问：NAS 上有什么照片？"] --> B["🔍 OpenClaw 匹配\nnas_search Skill"]
...
flowchart TB
    ...
    subgraph surface ["Surface - OpenClaw 调度层"]
        direction TB
        S1["飞书渠道收到消息"]
        S2["OpenClaw 匹配 bilibili_summary Skill"]
        S3["exec 调用 bilibili_summary.sh"]
        S4["读取返回的AI总结 转发给用户"]
        S1 --> S2 --> S3 --> S4
    end

    subgraph gpu ["3060 GPU 服务器 - FastAPI 8090"]
        direction TB
        G1["Step1 yt-dlp 获取视频信息"]
        G2["Step2 yt-dlp 下载视频"]
        G3["Step3 ffmpeg 提取音频"]
        G4["Step4 Docker GPU Whisper转写"]
        G5["Step5 Qwen3-32B AI总结"]
        G6["Step6 SSH回传文件到NAS"]
        G1 --> G2 --> G3 --> G4 --> G5 --> G6
    end
